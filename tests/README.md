...
Core tests take longer than any other coinevo tests, due to the high amount of computational work involved in validating core components.
...
To run only coinevo's core tests (after building):
...
To run only coinevo's crypto tests (after building):
...
```bash
coinevod --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `coinevo-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only coinevo's hash tests (after building):
...
To run only coinevo's performance tests (after building):
...
To run only coinevo's unit tests (after building):
